 ...
 *
 * $Id: RunProcessor.java,v 1.1 2005/06/21 02:40:32 vlad_r Exp $
 */
 ...
 */
public
final class RunProcessor extends Processor
                         implements IAppErrorCodes
{
     ...
     *
     * @param delegate loader that will be used for class delegation by the instrumenting
     *  class loader created by this RunProcessor [if null, delegation is disabled]
     *
     * @return a new Processor instance
     */
    public static RunProcessor create (final ClassLoader delegate)
    {
    ...
    public synchronized void run ()
    {
        ...
        // reset runtime [RT loads app properties, creates 'cdata', and launches the runtime controller if needed]:
        RT.reset (new RTSettings.SetActions (RTSettings.FIELD_NEW, RTSettings.FIELD_NEW, RTSettings.FIELD_NULL, RTSettings.FIELD_NEW_IF_NULL));
    ...
    protected void _run (final IProperties toolProperties)
    {
        ...
        {
            // clear all runtime state except for the controller:

            RT.reset (new RTSettings.SetActions (RTSettings.FIELD_NULL, RTSettings.FIELD_NULL, RTSettings.FIELD_NULL, RTSettings.FIELD_NEW_IF_NULL));
        }
    ...
    private RunProcessor (final ClassLoader delegate)
    {
        m_delegate = delegate; // allowed to be null
        m_coveragePath = IConstants.EMPTY_FILE_ARRAY;
    }
    ...
    private final ClassLoader m_delegate; // can be null for run() [in which case classloading delegation is disabled]
    ...
    // setting the context loader on RunProcessor's thread should not
