 ...
 *
 * $Id: InstrVisitor.java,v 1.6 2005/05/18 13:03:09 vlad_r Exp $
 */
                         ...
                         implements IClassDefVisitor, IAttributeVisitor, IOpcodes, IConstants
{
    ...
    {
        m_excludeEmptyClasses = options.excludeEmptyClasses ();
     ...
     * </ul>
     * This method uses a caller-provided out-parameter for returning results.
     * Note that if 'out.m_metadata' is not null 'out.m_instrumented' will almost
     * always be the same as 'instrument', with the exception of classes that
     * can't be instrumented (e.g, due to being at the JVM class size limits).<P>
     *
     * NOTE: if 'instrument' is 'true', the caller should always assume that 'cls'
     * has been mutated by this method. If the caller needs to recover the original
     * class definition, it should have created one as a <code>cls.clone()</code>
     * or by retaining the original definition bytes. This part of contract is
     * for efficienty and also simplifies the implementation.<P>
     *
     * TODO: "don't instrument" execution path isn't implemented yet (m_instrument
     * is not used)<P>
     * TODO: classes that can't be instrumented aren't detected yet<P>
     */
    ...
    public final Object visit (final ClassDef ignore, final Object ctx)
    {
        ...
        final IMethodCollection methods = cls.getMethods ();

        // SF FR 977738: skip empty classes (javac generates these for some code patterns)
        if (m_excludeEmptyClasses && (methods.size () == 0))
        {
            m_instrument = false;
            m_metadata = false;

            if (trace1) m_log.trace1 ("visit", "skipping empty class");
            return null;
        }
        ...
        visit (cls.getAttributes (), null);

        return null;
    }


    public final Object visit (final IMethodCollection methods, final Object ctx)
    {
        final ClassDef cls = m_cls;
    ...
    public final Object visit (final IAttributeCollection attributes, Object ctx)
    {
    ...
    public final Object visit (final CodeAttribute_info attribute, final Object ctx)
    {
        final boolean trace2 = m_log.atTRACE2 ();
    ...
    public final Object visit (final LineNumberTableAttribute_info attribute, final Object ctx)
    {
        final boolean trace2 = m_log.atTRACE2 ();
        final boolean trace3 = m_log.atTRACE3 ();
    ...
    public final Object visit (final ExceptionsAttribute_info attribute, final Object ctx)
    {
        return null;
    }

    public final Object visit (final ConstantValueAttribute_info attribute, final Object ctx)
    {
        return null;
    }

    public final Object visit (final SourceFileAttribute_info attribute, final Object ctx)
    {
        String value = attribute.getSourceFile (m_cls).m_value;

        // user experience shows that there are bytecode enhancers (or compilers)
        // out there that set SourceFile to an empty String (not valid .class
        // format)

        if ((value != null) && (value.length () > 0))
        {
            // BUG SF1002854: similarly, there are compilers that
            // set SourceFile value to the full source file pathname
            // (a la jvc 1.1); work around that by stripping the extra
            // pathname components:

            int lastSlash = value.lastIndexOf ('\\');
            if (lastSlash >= 0)
                value = value.substring (lastSlash + 1);
            else if ((lastSlash = value.lastIndexOf ('/')) >= 0)
                value = value.substring (lastSlash + 1);

            m_classSrcFileName = value;
        }

        return null;
    }

    public final Object visit (final SyntheticAttribute_info attribute, final Object ctx)
    {
        return null;
    }

    public final Object visit (final BridgeAttribute_info attribute, final Object ctx)
    {
        return null;
    }

    public final Object visit (final InnerClassesAttribute_info attribute, final Object ctx)
    {
        return null;
    }

    public final Object visit (final GenericAttribute_info attribute, final Object ctx)
    {
        return null;
    }
